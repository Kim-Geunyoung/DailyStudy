...
-- 변수 값 출력
-- 보통 변수 이름엔 @를 붙여주고 안 붙이면 컬럼 이름으로 인식
SET @Today = CURDATE();
SELECT @Today;
...
SELECT emp_id, gender, emp_name, hire_date
FROM employee
WHERE gender = 'M';
...
-- 이름 중앙에 '국'이 들어간 직원(국국국 같은 경우에도 출력이됨 그렇기 때문에 and하고 not like 써서 추가적인 조건 달아주기)
SELECT emp_id, emp_name, dept_id, hire_date, email, phone
	FROM employee
...
-- 2016년에 입사인 직원 정보 조회 -> 이렇게 작성해도 무관하지만 성능이 낮음 -> 권고하지 않음 -> where절에는 함수사용 안 하는게 좋음
SELECT emp_name, emp_id, dept_id, hire_date, phone
	FROM employee
...
select *
from employee
where emp_id IN ('SYS', 'MKT') and hire_date between '2014.01.01' and '2014.12.31';


-- Q) 2015년도에 입사한 연봉이 6,000 이상인 근무중인 직원 정보 조회
select *
from employee
where (salary >= 6000) and (hire_date between '2015.01.01' and '2015.12.31') and (retire_date is null);


-- Q) 홍길동(S0001), 강우동(S0003), 오삼식(S0005)의 2015년 휴가 정보 조회
select *
from vacation
where emp_id in ('S0001', 'S0003', 'S0005') and begin_date between '2015.01.01' and '2015.12.31';