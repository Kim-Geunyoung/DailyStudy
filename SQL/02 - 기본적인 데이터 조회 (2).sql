...
-- MySQL: IFNULL() 참고 : ISNULL() --> 전달된 값이 NULL이면 1, 아니면 0을 반환 -> MSSQL의 ISNULL과 이름이 같지만 다른 기능
-- MSSQL: ISNULL()
-- ORACLE: NVL()
*/

-- COALESCE() 함수: 나열된  값 중에서 첫 번째 NULL이 아닌 값 사용
...
-- 자동 형 변환 ->  문자열을 +로 합치지 못 한다. 자동으로 숫자형으로 변환하여 계산됨 -> 문자가 나오면 인식 못 하고 계산 안 한다.
SELECT '10' + '20';
SELECT 10 + '20'; 
SELECT 10 + '20AX';
SELECT 10 + 'LX20';
...
-- eng_name이 null이면 공백으로 표시하기 CASE문과 IF문 차이 비교(결과는 같음)
-- CASE문
SELECT emp_name, 
	CASE WHEN eng_name IS NULL THEN ''
    ELSE eng_name END AS eng_name,
