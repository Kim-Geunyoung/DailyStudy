...
-- 시각화 야매로 만들어 보기
select emp_name, dept_id, repeat('▒', round(salary / 250)) AS salary
from employee
where salary > 0;
...
- 집계 함수는 null값을 무시한다 -> 다른 것들 예를들면 +를 연산할 때 null이 있으면 null값을 반환하는데 집계함수는 null값 무시하고 집계 해줌
	- 집계 함수는 null값을 무시하지만 COUNT(*)은 null값을 무시하지 않는다.
*/

SELECT MAX(salary) AS tot_salary,
	MIN(salary) AS tot_salary,
    count(salary),
    count(emp_id),
    count(*)
	FROM employee
...
-- 참고1 : 이러면 dept_id에 여러개의 emp_name이 있는데 그 중 제일 앞에 나오는 정보를 보여줌 이렇게 하면 안돼!
SELECT dept_id, emp_name, COUNT(*) AS emp_count
	FROM employee
	WHERE retire_date IS NULL
	GROUP BY dept_id;

-- 참고2 : 참고 1처럼 하면 오류가 날 수도 있어서 이렇게 해야함
...
SELECT dept_id, 
       ...
       MAX(salary) - MIN(salary) AS diff_salary
	FROM employee
	WHERE retire_date IS NULLemployee
	GROUP BY dept_id;
    
-- pivot(크로스탭) 분석
SELECT dept_id, SUM(IF(gender='M', salary, 0)) AS M_salary, SUM(IF(gender='F', salary, 0)) AS F_salary, SUM(salary) AS total
FROM employee
WHERE retire_date IS NULL
group by dept_id;
...
select dept_id, sum(salary) AS tot_sal
from employee
where retire_date IS null
group by dept_id
...
-- HAVING 절에 열 별칭 사용 가능(권고하지 않음)-> 다른 SQL에서는 오류남: having이 select보다 빨리 실행되는데
-- emp_count가 선언되기 전에 사용되기 때문
SELECT dept_id, COUNT(*) AS emp_count
	FROM employee
	WHERE retire_date IS NULL
	GROUP BY dept_id
...
select emp_id, count(*) as count_duration
from vacation
where begin_date between '2017.01.01' and '2017.12.31'
group by emp_id
having count(*) >= 3;
